...
function get_branch_asset_count($branch_id)
{
    $sql = "SELECT COUNT(*) as count FROM tu_branch_asset WHERE branch_id = $1";
    $result = db_fetch_all($sql, [$branch_id]);
    return $result[0]['count'] ?? 0;
}

function get_branch_asset_paginated($branch_id, $limit, $offset)
{
    $sql = "SELECT * FROM tu_branch_asset WHERE branch_id = $1 ORDER BY name LIMIT $2 OFFSET $3";
    return db_fetch_all($sql, [$branch_id, $limit, $offset]);
}
